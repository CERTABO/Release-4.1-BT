...
from utils import media, logger, reader_writer, usbtool, bluetoothtool
    ...
    while True:
        ...
        if certabo_settings['address_chessboard'] is None:
            if certabo_settings['connection_method'] == 'usb':
                ...
                # Bluettoth find_adress() has to be launched on another thread, as it takes a long time to run
                if bt_find_address_executor is None:
                    bt_find_address_executor = ThreadPoolExecutor(max_workers=1)

                if future_bt_address is None:
                    # ThreadPoolExecutor runs threads as non-daemon, so program may take a while to exit if bluetoothool.find_address is still running
                    future_bt_address = bt_find_address_executor.submit(bluetoothtool.find_address, test_address=last_address_chessboard)
                else:
                    if future_bt_address.done():
                        certabo_settings['address_chessboard'] = future_bt_address.result()
                        future_bt_address = None
        ...
        if chessboard_connection_process is None and certabo_settings['address_chessboard'] is not None:
            if certabo_settings['connection_method'] == 'usb':
                ...
                chessboard_connection_process = bluetoothtool.start_bluetoothtool(certabo_settings['address_chessboard'], separate_process=True)
